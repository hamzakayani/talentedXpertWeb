...
const ProfileSetting = () => {
    ...
    const { register, setValue, getValues, control, handleSubmit, formState: { errors, } } = useForm<FormSchematype>({
        defaultValues: {
            ...
            profileType: user?.profile?.length > 0 && user?.profile?.length> 0 && user?.profile[0]?.type,
            userType: "INDIVIDUAL",
            skills: [],
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="card-body bg-gray">
                        <div className='container'>
                            ...
                            <div className='row'>
                                <div className='col-md-6'>
                                    {user?.profile?.length > 0 && user?.profile?.length> 0 && user?.profile[0]?.type=== 'TE'&&<div className="mb-3">
