"use client";
import {
  useFetchAllProposals,
  useFindTopProposal,
} from "@/hooks/proposals/useProposal";
...
const ProposalsList = () => {
  ...
  const fetchProposals = useFetchAllProposals({
    params: {
  ...
  const getTopProposals = async () => {
    if (fetchProposals?.data?.data?.proposals?.length > 0) {
      const data = {
        job_description:
          fetchProposals?.data?.data?.proposals?.[0]?.task?.details,
        proposals: Object.fromEntries(
          fetchProposals?.data?.data?.proposals?.map((prop: any) => [
            prop?.id,
            prop?.details,
          ]) || []
        ),
      ...
      try {
        const response = await topProposalMutation.mutateAsync(data);
    ...
    >
      ...
      >
        <p className="m-0 fw-medium">
          Received Proposals (
          {fetchProposals?.data?.data?.proposals?.length ?? 0})
        </p>
        <Icon
        ...
        <div className="p-3">
          ...
          <div className="row row-gap-2 row-gap-md-4 my-3">
            {fetchProposals?.isLoading || topProposalMutation?.isPending ? (
              <SpinnerLoader />
            ) : (!fetchProposals?.isLoading ||
                !topProposalMutation?.isPending) &&
              fetchProposals?.data?.data?.proposals?.length > 0 ? (
