    ...
    <div className="new-card" onClick={() => router.push(`/dashboard/tasks/${task?.id}`)} style={{ cursor: "pointer" }}>
      ...
      <div className="requester-info">
        <div className="requester-left">
          ...
          {/* Rating */}
          <div className="rating-text">{task?.requesterProfile?.averageRating || 4}/5 Requestor Rating</div>
          {/* Star Rating */}
          <div className="star-rating">
            {[...Array(Math.floor(task?.requesterProfile?.averageRating  || 4))].map((_, i) => (
              <Icon key={i} icon="solar:star-bold" className="star-filled" />
            ))}
            {[...Array(5 - Math.floor(task?.requesterProfile?.averageRating  || 4))].map((_, i) => (
        ...
        <div className="requester-stats">
          <div className="stat-item">Total tasks: {task?.expertsHired || 400}+</div>
