...
const ProposalCard: FC<ProposalCardProps> = ({
  ...
  return (
    <div
      className={`promoted_task new-card mb-2 d-flex flex-column h-100 ${
        isDashboard
          ? `bg-dashbord-card p-4 relative ${
              data?.expertProfile?.user?.disability ? "border-gradient3" : ""
            }`
          : "p-4"
      }`}
    ...
    >
      {data?.expertProfile?.user?.disability && (
        <div
          className={`disablity bg-gradient3 rounded-5 w-auto py-1 px-4 d-flex align-items-center gap-2 maxw-auto position-absolute ${
            isDashboard ? "text-dark" : ""
          }`}
        >
<<<<<<< HEAD
        ...
        <div className="d-flex align-items-start justify-content-between">
          ...
          >
            ...
            <Link
              ...
              style={{ minWidth: 0 }}
              href={`${isDashboard ? "/dashboard" : ""}/talented-xperts/${
                data?.expertProfile?.id
              }`}
              onClick={() =>
                navigate(
                  `${isDashboard ? "/dashboard" : ""}/talented-xperts/${
                    data?.expertProfile?.id
                  }`
                )
              }
                    ...
                    <div className="d-flex flex-column align-items-end">
                        ...
                        <span
                            className={`badge text-end
                                ${
                                data?.status === "HIRED"
>>>>>>> 3f29f98c3517e329d297374d217afeeb8ce297ca
                                ? "text-bg-success"
      ...
      <div className="d-flex justify-content-between align-items-center flex-wrap ">
        ...
        <div className="d-flex justify-content-between align-items-center mb-md-1">
          <span
            ...
            style={{ gap: 5 }}
          >
            <HugeiconsIcon icon={Calendar04Icon} width={17} height={17} />
            Submitted{" "}
          </span>
          <p className="m-0 ms-1 fs-12">{getTimeago(data?.createdAt)}</p>
        </div>
        ...
        <div className="d-flex justify-content-between align-items-center mb-md-1">
          ...
          <p className="m-0 ms-1 fs-12">${data?.amount}</p>
        </div>
      </div>
      ...
      <div className="requester-info">
        <div className="requester-left">
          ...
          <div className="star-rating">
            ...
            {[
              ...Array(5 - Math.floor(data?.expertProfile?.averageRating || 0)),
            ].map((_, i) => (
              <Icon
                key={i}
        ...
        <div className="requester-stats">
          {btn ? (
            <Link
              className="btn btn-outline-light rounded-pill btn-sm w-auto mt-1 ff-figtree fw-normal"
              style={{ textAlign: "center" }}
              href={`${isDashboard ? "/dashboard" : ""}/tasks/${
                data?.task?.id
              }/proposals/${data?.id}`}
              onClick={() =>
                navigate(
                  `${isDashboard ? "/dashboard" : ""}/tasks/${
                    data?.task?.id
                  }/proposals/${data?.id}`
                )
              }
