...
const NewVideoCall: FC<{
    ...
    //   useEffect(() => {
    //     console.log('VideoCall mounted:', { isCaller, userName, threadId: thread?.id });
    //     setToken(null);
    //     setMeetingId(null);
    //     setError(null);
    //     setCallStatus(isCaller ? 'ringing' : 'ringing');
    ...
    const initiateCall = useCallback(async () => {
        if (!thread?.id || token || meetingId || !socket) {
            ...
            //             isInitiating ? 'Call initiation in progress' :
            //                 null
            // );
            return;
        }
    ...
    }, [socket, thread, token, meetingId, userName, isInitiating]);

    useEffect(() => {
        if (isCaller && !isInitiating) {
            console.log('Triggering initiateCall for caller');
            initiateCall();
        }
    }, [isCaller, initiateCall, isInitiating]);
...
// MeetingView and ParticipantView remain unchanged
