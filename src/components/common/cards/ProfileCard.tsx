...
export default function ProfileCard() {
  ...
  return (
    <div className={`profile-card position-relative p-3 rounded border-gradient3`} style={{ height: '145px !important' }}>
      {user?.disability && (
              ...
              <div className="rating">
                {[...Array(5)].map((_, index) => (
                  <Icon
                    ...
                    className={`${index < user?.profile?.[0]?.averageRating ? "rated" : ""}`}
                  />
                ))}
              </div>
              <small className="text-white mt-1">{user?.profile?.[0]?.averageRating}/5</small>
            </div>
          </div>
        </div>
        <div 
