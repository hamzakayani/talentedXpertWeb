...
const SubmitReviewMilestone = ({
  setsubmitReviewMilestoneCheck,
  reviewMilestone,
  taskId,
  revieweeTeamId,
  getContract,
  ...
  const {
    ...
    defaultValues: {
      ...
      milestoneId:
        task?.amountType === "FIXED" ? Number(reviewMilestone.id) : undefined,
      reviewerProfileId: Number(user?.profile?.[0]?.id),
      revieweeProfileId: Number(
        reviewMilestone.teamMemberProfileId || reviewMilestone.teProfileId
      ),
      revieweeTeamId: revieweeTeamId ? Number(revieweeTeamId) : undefined,
      weeklyMilestoneId:
        task?.amountType === "HOURLY" ? Number(reviewMilestone.id) : undefined,
    },
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="d-flex justify-content-center align-items-center gap-1">
          <label htmlFor="exampleFormControlInput1" className="fs-14">
            Add Ratings :
          </label>
          <Controller
            name="rating"
              ...
              <div className="stars d-flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <HugeiconsIcon
                    key={star}
                    icon={StarIcon}
                    className={star <= field.value ? "text-warning" : ""}
                    onClick={() => {
                      field.onChange(star);
